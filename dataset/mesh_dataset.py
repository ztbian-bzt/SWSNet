...
class Teeth3DSDataset(Dataset):
    ...
    def _donwscale_mesh(self, mesh, labels):
        target_point = 16000  # 16000 / 32000 / 64000
        mesh_simplifier = pyfqmr.Simplify()
        mesh_simplifier.setMesh(mesh.vertices, mesh.faces)
        mesh_simplifier.simplify_mesh(target_count=target_point, aggressiveness=3, preserve_border=True, verbose=0,
                                      max_iterations=2000)
        ...
        if faces.shape[0] < target_point:
            fs_diff = target_point - faces.shape[0]
            faces = np.append(faces, np.zeros((fs_diff, 3), dtype="int"), 0)
        elif faces.shape[0] > target_point:
            mesh_simple = trimesh.Trimesh(vertices=vertices, faces=faces)
            samples, face_index = trimesh.sample.sample_surface_even(mesh_simple, target_point)
