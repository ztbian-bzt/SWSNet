...
class DBGANet(nn.Module):
    def __init__(self, num_classes=17, num_neighbor=20):
        super(DBGANet, self).__init__()

        self.k = num_neighbor

        self.stn_c1 = STN3d(k=3)
        ...
        self.pe1 = nn.Sequential(nn.Linear(48, 64), nn.ReLU())
        self.pe2 = nn.Sequential(nn.Linear(48, 128), nn.ReLU())
        self.pe3 = nn.Sequential(nn.Linear(48, 256), nn.ReLU())
    ...
    def forward(self, x):
        ...
        coor1 = coor1 + cent1
        coor1 = self.transformer1(coor1, 160, 100, 2)
        ...
        coor2 = coor2 + cent2
        coor2 = self.transformer2(coor2, 160, 100, 2)
        ...
        coor3 = coor3 + cent3
        coor3 = self.transformer3(coor3, 160, 100, 2)
...
class LitDBGANetwork(L.LightningModule):
    def __init__(self):
        super().__init__()
        self.model = DBGANet(num_classes=17, num_neighbor=32)
    ...
    def training_step(self, batch, batch_idx):
        x, y, centroids, y_classes = batch
        B, N, _ = x.shape

        y = y.view(B, N).float()
        pred, cents, classes, weight = self.model(x.transpose(2, 1))
        pred1 = pred.reshape(-1, 17)
        y1 = y.view(-1, 1).squeeze(1)
        seg_loss = F.nll_loss(pred1, y1.long())
        cent_loss = F.smooth_l1_loss(cents, centroids)
        class_loss = self.cross_entropy(classes, y_classes[:, 0, :])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y = y.view(B, N).float()
        pred, cents, classes, weight = self.model(x.transpose(2, 1).contiguous())
        pred1 = pred.reshape(-1, 17)
        y1 = y.view(-1, 1).squeeze(1)
