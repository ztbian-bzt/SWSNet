...
def downsample_mesh(mesh: trimesh, labels: np.ndarray) -> trimesh:
    mesh = mesh.copy()
    colors = downsample_to_colors(labels)
    mesh.visual.face_colors = colors
    return mesh


def downsample_to_colors(labels: np.ndarray) -> np.ndarray:
    sorted_index = sorted(list(_downsample_color.items()), key=lambda tup: tup[0])
    class_colors = np.array([i[1] for i in sorted_index])
    return class_colors[labels]
